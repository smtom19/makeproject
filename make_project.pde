...
void setup(){
  ...
  hero = new Player(150, 524, 50);  

  for(int i = 0; i < 300; i++){
    obstacles[i] = new Obstacle(1000); //1000 signifies right side of game
  }
}
...
void draw(){
  ...
  //obstacles
  for(int i = 0; i < 300; i++){
    obstacles[i].move(4); //starting speed
  }
  obstacleSpawn();  
  collision(); 
  ...
  for(int i = 0; i < 300; i++){
    if(hero.getX() > obstacles[i].spikeGetX1() && hero.getX() < obstacles[i].spikeGetX2()){
      if(hero._PlayerY > obstacles[i].spikeGetY1() && hero._PlayerY < obstacles[i].spikeGetY2()){
        println("Oh no! The spikes killed you.");
    ...
    if(hero.getX() > obstacles[i].squareGetX1() && hero.getX() < obstacles[i].squareGetX2()){
      if(hero._PlayerY > obstacles[i].squareGetY1() && hero._PlayerY < obstacles[i].squareGetY2()){
        println("Oh no! The spikes killed you. Be careful next time.");
        delay(1000);
        reset();
      }
...
void scoreboard(){ 
  ...
  textSize(35);
  fill(0);
  text("High Score: "+highScore/60 +" seconds", 800, 50); //high score is calculated by seconds in the game
  
  //deaths 
  fill(0);
  textSize(35);
  text("Deaths:" +deathCounter, 475, 50);
}

void displayControls(){
  //displays controls until the user reaches the first obstacle
  if(timer < 250){
    textAlign(CENTER);
    textSize(24);
    fill(0);
    text("To Jump: Press Up or Spacebar", 200, 50);
...
void background(){
  ...
  noStroke();
  fill(255,255,255);
  ellipse(200,150,40,15);
  ellipse(200,150-8,20,10);
    
  fill(255,255,255);
  ellipse(500,100,40,15);
  ellipse(500,100-8,20,10);
    
  fill(255,255,255);
  ellipse(800,130,40,15);
  ellipse(800,130-8,20,10);
...
void gameComplete(){
  ...
  text("Congrats, You Win The Game!", 500, 300);
}
